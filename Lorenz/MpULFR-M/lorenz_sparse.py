...
def prepare_data(libdata):
    ...
    w_vec = tf.ones([row,col])
    lammda = np.c_[u_vec,v_vec, w_vec]
    lambda_T = lammda.T   #dtype:float32
    lambda_T = tf.cast(lambda_T,dtype=tf.float64) #convert tensor to tensor
    #print(matrix_T)
    y_coeff = tf.cast(tf.ones([3,row])/10,dtype=tf.float64)
...
#----------------end tuning of learning rate--------
    ...
    lambda_LU = np.vstack((tf.ones([col,row]), tf.ones([col,row]), tf.ones([col,row])))
...
def get_admm_sol(libdata):
    ...
    sol_lambda = scint.solve_ivp(fun=func_check, t_span=(t0,t1), y0=y0, method="LSODA", t_eval=np.linspace(t0,t1,gap))
...
def get_LU_sol(libdata):
    ...
    sol_LU = scint.solve_ivp(fun=func_check, t_span=(t0,t1), y0=y0, method="LSODA", t_eval=np.linspace(t0,t1,gap))
...
def get_lasso_sol(libdata):
    ...
    def func_check(t, x):
        ...
        y3 = w[3,2] + w[0,2]*x[0] + w[1,2]*x[1] + w[2,2]*(x[2])# + w[3,1]*(x[1]**2) + w[4,1]*(x[0]*x[1])
        #---------------lasso----------
    ...
    sol_sparse = scint.solve_ivp(fun=func_check, t_span=(t0,t1), y0=y0, method="LSODA", t_eval=np.linspace(t0,t1,gap))
...
def visualize(libdata):
    ...
    # ax4.plot(output[:,0], output[:,1],output[:,2],'x', label='Data(LSODA)')  
    # ax4.legend(loc='upper left')
    # ax4.plot(sparse_u_pinn[:,0],sparse_u_pinn[:,1],sparse_u_pinn[:,2], label='Data(PINNs)')  
    ...
    y = output[:,1]
    z = output[:,2]
    ax4.plot(x, z, lw=0.6, color='blue', label='Data(LSODA)')
    x1 = sparse_u_pinn[:,0]
    y1 = sparse_u_pinn[:,1]
    z1 = sparse_u_pinn[:,2]
    ax4.plot(x1, z1, linestyle='--', color='red', label='Data(PINNs)')
    ...
    # ax5.plot(output[:,0], output[:,1],output[:,2],'x', label = 'Data(LSODA)')    #(100,2)
    # ax5.legend(loc='upper left')
    # ax5.plot(sol_lambda.y[0,:],sol_lambda.y[1,:],sol_lambda.y[2,:], label='IPINNs')
    ...
    y = output[:,1]
    z = output[:,2]
    ax5.plot(x, z, lw=0.6, color='blue', label='Data(LSODA)')
    x1 = sol_lambda.y[0,:]
    y1 = sol_lambda.y[1,:]
    z1 = sol_lambda.y[2,:]
    ax5.plot(x1, z1, linestyle='--', color='red', label='Data(IPINNs)')
    ax5.set_xlabel("x",fontsize=12)
    ax5.set_ylabel("z",fontsize=12)
    ax5.legend()
    #ax7.set_title("Lorenz x–z Projection", fontsize=12)
    plt.tight_layout()
    plt.savefig('IPINNs.png', dpi=300)
    plt.show()
    
    # fig,ax6 = plt.subplots(1,1)
    # #fig.suptitle("RK45 vs SIPINNs", fontsize=12)
    # ax6.plot(output[:,0], output[:,1],output[:,2],'x', label = 'Data(LSODA)')    #(100,2)
    # ax6.legend(loc='upper right')
    # ax6.plot(sol_LU.y[0,:],sol_LU.y[1,:], sol_LU.y[2,:], label='Data(SIPINNs)')
    ...
    y = output[:,1]
    z = output[:,2]
    ax6.plot(x, z, lw=0.6, color='blue', label='Data(LSODA)')
    x1 = sol_LU.y[0,:]
    y1 = sol_LU.y[1,:]
    z1 = sol_LU.y[2,:]
    ax6.plot(x1, z1, linestyle='--', color='red', label='Data(SIPINNs)')
    ...
    # ax7.plot(output[:,0], output[:,1],output[:,2],'x', label = 'Data(LSODA)')    #(100,2)
    # ax7.legend(loc='upper right')
    # ax7.plot(sol_sparse.y[0,:],sol_sparse.y[1,:],sol_sparse.y[2,:], label='Data(lasso)')
    ...
    y1 = sol_sparse.y[1,:]
    z1 = sol_sparse.y[2,:]
    ax7.plot(x1, z1, linestyle='--', color='red', label='Data(lasso)')
    ...
    ax10.plot(t, output[:,2],'x', label = 'exact_z')
    ax10.plot(t, sol_lambda.y[2,:], label = 'z')
    #ax9.plot(t[0:-1], sol_lambda.y[1,:], 'x', label = 'y')
...
def save_output(libdata, fname="lor_sp.pkl"):
...
def main():
    ...
    save_output(libdata, fname="lor_sp.pkl")
