...
def EM_mlr(libdata):
    ...
    iv = np.c_[const_one,x, y,z, x2, y2, xy, xz ]
    ...
    beta1 = em_algorithm_for_multiple_linear_regression(iv, dxdt)
    print(f"Estimated em_dxdt Coefficients: {beta1}")
    
    beta2 = em_algorithm_for_multiple_linear_regression(iv, dydt)
    print(f"Estimated em_dydt Coefficients: {beta2}")  
    
    beta3 = em_algorithm_for_multiple_linear_regression(iv, dzdt)
    print(f"Estimated em_dydt Coefficients: {beta3}")  
...
def get_mlr_sol(libdata):
    ...
    sol_em_mlr = scint.solve_ivp(fun=func_check, t_span=(t0,t1), y0=y0, method="RK45", t_eval=np.linspace(t0,t1,gap))
...
def visualize(libdata):
    ...
    # ax1.plot(sol_func.y[0,:],sol_func.y[1,:], label='Data(MpULFR-0)')
    # ax1.set_xlabel('Displacement, x(t)')#, fontdict=font)
    # ax1.set_ylabel("Velocity, x'(t)")#, fontdict=font)
    # ax1.legend(loc='upper left')
    ...
    plt.tight_layout()
    plt.savefig('lorenz_mlr.png', dpi=300)
    plt.show()
