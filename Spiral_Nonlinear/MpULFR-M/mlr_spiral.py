# -*- coding: utf-8 -*-
"""
Created on Sat Sep 27 14:26:35 2025
...
def coupled_ode(t, z):
    x, y = z
    dxdt = -y + miu*(x**2)*(1-((x**2)+(y**2)))      
    dydt = x + miu*y*(1-((x**2)+(y**2))) 
    return [dxdt, dydt]
...
# -------------------------
t_span = (0, 10)
t_eval = np.linspace(t_span[0], t_span[1], 200)  # fixed 200 points

# -------------------------
# Solve the true ODE
# -------------------------
sol = solve_ivp(coupled_ode, t_span, [-0.15, 0.03], dense_output=True)
...
# Features: [1, x, y, x*y]
#X_features = np.vstack([np.ones_like(x), x, y, x * y]).T
...
def mlr_ode(t, z):
    x, y = z
    #features = np.array([1, x, y, x*y]).reshape(1, -1)
...
# -------------------------
mlr_sol = solve_ivp(mlr_ode, t_span, [-0.15, 0.03], t_eval=t_eval)
...
plt.legend()
#plt.axis('equal')
plt.savefig("spiral_mlr.png", dpi=300, bbox_inches="tight")
plt.show()
