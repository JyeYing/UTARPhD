...
def setting_model(data):
    ...
    model.add(Dense(32, activation=tf.keras.activations.gelu, kernel_initializer="glorot_uniform", dtype=tf.float64))
    model.add(Dense(64, activation=tf.keras.activations.gelu, kernel_initializer="glorot_uniform", dtype=tf.float64))
    model.add(Dense(32, activation=tf.keras.activations.gelu, kernel_initializer="glorot_uniform", dtype=tf.float64))
    model.add(Dense(16, activation=tf.keras.activations.gelu, kernel_initializer="glorot_normal", dtype=tf.float64))
    model.add(Dense(8, activation=tf.keras.activations.gelu, kernel_initializer="glorot_uniform", dtype=tf.float64))
...
def visualize(data):
    ...
    ax1.plot(numerical_neighbour.y[0,:], numerical_neighbour.y[1,:],label='Data(RK45)')
    ax1.legend(loc='upper left')
    ax1.plot(u_pred[:,0],u_pred[:,1],'x',label='Data(PINNs)')
    ax1.legend(loc='upper left')
    plt.savefig("spiral_upred.png", dpi=300, bbox_inches="tight")
    plt.show()

    fig,ax2 = plt.subplots(1,1)
    fig.suptitle("RK45 vs PINNs", fontsize=12)
    ax2.plot(numerical_sol.y[0,:], numerical_sol.y[1,:], label='Data(RK45)')
    ax2.legend(loc='upper left')
    ax2.plot(u_test[:,0],u_test[:,1],'x',label='Data(PINNs)')
    ax2.set_xlabel('x')#, fontdict=font)
    ax2.set_ylabel('y')#, fontdict=font)
    ax2.legend(loc='upper left')
    plt.savefig("spiral_test.png", dpi=300, bbox_inches="tight")
    plt.show()

def save_output(data, fname="recode_spiral.pkl"):
...
def main():
    ...
    visualize(data)
    save_output(data, fname="recode_spiral.pkl")
