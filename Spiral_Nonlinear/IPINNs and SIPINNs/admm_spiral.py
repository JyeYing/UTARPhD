...
def pinn(libdata):
    ...
    model = load_model('record_model.keras',custom_objects={'custom_loss': custom_loss})
    model.summary()
...
def prepare_data(libdata):
    ...
    array = tf.stack([const_one,x,y,x2,y2,xy],axis=0)
    #print(array)   #(5x100)
...
def lass_fit(libdata):
    ...
    lasso1 = Lasso(alpha=α1, fit_intercept=True, tol=1e-4, max_iter=500)
    lasso1.fit(iv,dxdt)
    ...
    lasso2 = Lasso(alpha=α2, fit_intercept=True, tol=1e-4, max_iter=500)
    lasso2.fit(iv,dydt)
...
def get_admm_sol(libdata):
    ...
    sol_lambda = scint.solve_ivp(fun=func_check, t_span=(0,10), y0=[-0.15,0.03], method="RK45", t_eval=np.linspace(0,10,500))
...
def get_LU_sol(libdata):
    ...
    sol_LU = scint.solve_ivp(fun=func_check, t_span=(0,10), y0=[-0.15,0.03], method="RK45", t_eval=np.linspace(0,10,500))
...
def get_lasso_sol(libdata):
    ...
    sol_sparse = scint.solve_ivp(fun=func_check, t_span=(0,10), y0=[-0.15,0.03], method="RK45", t_eval=np.linspace(0,10,500))
...
def visualize(libdata):
    ...
    ax4.plot(output[:,0], output[:,1], 'x', label='Data(RK45)')  
    ax4.legend(loc='upper left')
    ax4.plot(sparse_u_pinn[:,0],sparse_u_pinn[:,1], label='Data(PINNs)')  
    ...
    ax5.plot(output[:,0], output[:,1],'x', label = 'Data(RK45)')    #(100,2)
    ...
    ax6.plot(sol_LU.y[0,:],sol_LU.y[1,:], label='Data(SIPINNs)')
    ...
    ax7.plot(sol_sparse.y[0,:],sol_sparse.y[1,:], label='Data(lasso)')
    ...
    ax8.plot(t, output[:,0],'x', label = 'Data(RK45)_x')
    ax8.plot(t, sol_lambda.y[0,:], label = 'Data(IPINNs)_x')
    ...
    ax9.plot(t, output[:,1], 'x', label = 'Data(RK45)_y')
    ax9.plot(t, sol_lambda.y[1,:], label = 'Data(IPINNs)_y')
    ...
    fig.savefig('spiral_ipinnsLoss.png', dpi=300)
    
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    # Displacement
    axes[0].plot(t, output[:,0],'x', label = 'Data(RK45)_x') #2
    axes[0].legend(loc='lower right')
    axes[0].plot(t, sol_lambda.y[0,:], label = 'Data(IPINNs)_x') #
    axes[0].legend(loc='lower right')
    ...
    # Phase space
    axes[1].plot(t, output[:,1], 'x', label = 'Data(RK45)_y') 
    axes[1].legend(loc='upper left')
    axes[1].plot(t, sol_lambda.y[1,:], label = 'Data(IPINNs)_y')
...
def save_output(libdata, fname="admm_spiral.pkl"):
...
def main():
    ...
    save_output(libdata, fname="admm_spiral.pkl")
