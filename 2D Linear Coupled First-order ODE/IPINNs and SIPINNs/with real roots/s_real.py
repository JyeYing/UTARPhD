...
α1 = 0.01 #lasso weight search for dxdt
α2 = 0.01 #lasso weight search for dydt 
...
def prepare_data(libdata):
    ...
    #print(matrix_T)
    y_coeff = tf.cast(tf.ones([2,3])/10,dtype=tf.float64)
...
#----------------end tuning of learning rate--------
    ...
    lambda_LU = np.vstack((tf.ones([1,3]), tf.ones([1,3])))
...
#_________to update yij_______
    ...
    #L = min 1/2 ||lambda_T*phi - ddt||^2 + lam ||z|| + sum y_ij(lambda_T-z_T) + rho/2 ||lambda_T-z_T||^2
        ...
        lamT_dx = np.array(lambda_T[0,:]).reshape(-1,1)
        lamT_dy = np.array(lambda_T[1,:]).reshape(-1,1)
        
        y_predict1 = XX @ lamT_dx 
...
def get_admm_sol(libdata):
    ...
    sol_lambda = scint.solve_ivp(fun=func_check, t_span=(0,10), y0=[-0.1,0.3], method="LSODA", t_eval=np.linspace(0,10,99))
...
def visualize(libdata):
    ...
    #fig.suptitle("IPINNs_real", fontsize=12)
    ...
    #fig.suptitle("SIPINNs_real", fontsize=12)
...
def save_output(libdata, fname="real_sparse.pkl"):
...
def main():
    ...
    save_output(libdata, fname="real_sparse.pkl")
