# -*- coding: utf-8 -*-
"""
Created on Sun Mar 23 16:25:55 2025
...
rho = ρ = tf.constant(20, dtype=tf.float64)
learn_rate = -2e-6
losses_sparse = []
...
α1 = 1 #lasso weight search for dxdt
α2 = 1 #lasso weight search for dydt 
αf1 = 10000 #loss func cal for dxdt
αf2 = 1e-4 #loss func cal for dydt

data = pd.read_csv("ode_dataRHS_repeated_roots_1.csv")
...
def make_data():    
    with open ('p_repeated_1.pkl','rb') as f:
        pdata = pickle.load(f)
...
def pinn(libdata):
    ...
    model = load_model('pinn_model_1.keras',custom_objects={'custom_loss': custom_loss})
    model.summary()
...
#----------------end tuning of learning rate--------
    ...
    ut_phi = tf.matmul(tf.transpose(ddt), tf.transpose(array))
...
def get_admm_sol(libdata):
    ...
    sol_lambda = scint.solve_ivp(fun=func_check, t_span=(0,10), y0=[-0.1,0.3], method="RK45", t_eval=np.linspace(0,10,99))
...
def get_LU_sol(libdata):
    ...
    def func_check(t, x):
        ...
        y1 =  -0.00035245 -0.096231*x[0] + 0.1979*x[1] #+ lams[0,3]*(x[0]**2) + lams[0,4]*(x[1]**2) #+ lams[0,5]*(x[0]*x[1]) 
        y2 =  -0.00012431 -0.12213*x[0] - 0.30658*x[1] #+ lams[1,3]*(x[0]**2) + lams[1,4]*(x[1]**2) #+ lams[1,5]*(x[0]*x[1])
    ...
    sol_LU = scint.solve_ivp(fun=func_check, t_span=(0,10), y0=[-0.1,0.3], method="LSODA", t_eval=np.linspace(0,10,99))
...
def get_lasso_sol(libdata):
    ...
    def func_check(t, x):
        ...
        #---------------sparse----------
        y1 = w[0,0]*x[0] + w[1,0]*x[1] + w[2,0]*(x[0]**2) + w[3,0]*(x[1]**2) + w[4,0] #+w[4,0]*(x[0]*x[1]) 
        y2 = w[0,1]*x[0] + w[1,1]*x[1] + w[2,1]*(x[0]**2) + w[3,1]*(x[1]**2) + w[4,1]#w[4,1]*(x[0]*x[1])
...
def visualize(libdata):
    ...
    #fig.suptitle("IPINNs_repeated", fontsize=12)
    ax6.plot(output[:,0], output[:,1],'x', label = 'Data(RK45)')    #(100,2)
    ax6.legend(loc='upper right')
    ax6.plot(sol_LU.y[0,:],sol_LU.y[1,:],label='Data(IPINNs)')
    ...
    ax7.plot(sol_sparse.y[0,:],sol_sparse.y[1,:], label='lasso')
