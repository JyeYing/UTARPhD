...
row = 3   #change if more variables added
col = 1
...
def make_data():    
    with open ('p_complex.pkl','rb') as f:
        pdata = pickle.load(f)
...
def various_opt(libdata):
    ...
    loss_sparsess = tf.reduce_mean(tf.square(tf.transpose(ddt) - lambda_phis))
...
def get_LU_sol(libdata):
    ...
    sol_LU = scint.solve_ivp(fun=func_check, t_span=(0,10), y0=[-0.1,0.3], method="RK45", t_eval=np.linspace(0,10,99))
...
def visualize(libdata):
    ...
    ax4.plot(output[:,0], output[:,1], label='Data(RK45)')  
    ...
    #fig.suptitle("IPINNs_complex", fontsize=12)
    ...
    #fig.suptitle("SIPINNs_complex", fontsize=12)
    ...
    ax7.legend(loc='upper right')
    fig.savefig('lasso.png', dpi=300)
     
    fig,ax8 = plt.subplots(1,1)
    ax8.plot(t, output[:,0], label = 'exact_x')
    ax8.plot(t[0:-1], sol_LU.y[0,:], 'x', label = 'x')
    ...
    ax9.plot(t[0:-1], sol_LU.y[1,:], 'x', label = 'y')
...
def save_output(libdata, fname="complex_sparse.pkl"):
...
def main():
    ...
    save_output(libdata, fname="complex_sparse.pkl")
