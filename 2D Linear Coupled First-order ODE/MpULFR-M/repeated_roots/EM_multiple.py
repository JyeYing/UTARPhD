...
def get_mlr_sol(libdata):
    ...
    sol_em_mlr = scint.solve_ivp(fun=func_check, t_span=(0,10), y0=[-0.1,0.3], method="RK45", t_eval=np.linspace(0,10,99))
...
def visualize(libdata):
    ...
    ax1.plot(sol_func.y[0,:],sol_func.y[1,:], label='Data(MpULFR_0)')
    ax1.set_xlabel('x')#, fontdict=font)
    ax1.set_ylabel('y')#, fontdict=font)
    ax1.legend(loc='upper right')
    fig.savefig('mlr_repeated_1.png', dpi=300)

    fig,ax2 = plt.subplots(1,1)
    #fig.suptitle("MpULFR-M_complex", fontsize=12)
    ax2.plot(output[:,0], output[:,1],'x', label = 'Data(RK45)')    #(100,2)
    ax2.legend(loc='upper right')
    ax2.plot(sol_em_mlr.y[0,:],sol_em_mlr.y[1,:], label='Data(MpULFR_0)')
