...
def sing_eigen_values(libdata):
    ...
    original_mat = np.array([[-0.5, 1], [-2, -0.5]])
...
def visualize(libdata):
    ...
    ax5.plot(sol_lambda.y[0,:],sol_lambda.y[1,:],'x', label='ADMM_non_sparse')
    ax5.set_xlabel('x')#, fontdict=font)
    ax5.set_ylabel('y')#, fontdict=font)
    ax5.legend(loc='upper right')
    ...
    ax6.plot(sol_LU.y[0,:],sol_LU.y[1,:],'x', label='ADMM_sparse')
...
def save_output(libdata, fname="ps_complex.pkl"):
...
def main():
    ...
    visualize(libdata)
    save_output(libdata, fname="ps_complex.pkl")
