...
def visualize(libdata):
    ...
    ax5.plot(output[:,0], output[:,1],'x', label = 'data')    #(100,2)
    ...
    ax6.plot(output[:,0], output[:,1],'x', label = 'data')    #(100,2)
    ax6.legend(loc='upper right')
    ax6.plot(sol_LU.y[0,:],sol_LU.y[1,:],'x', label='IPINNs')
    ax6.set_xlabel('x(t)')#, fontdict=font)
    ax6.set_ylabel('y(t)')#, fontdict=font)
    ax6.legend(loc='upper right')
