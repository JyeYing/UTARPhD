...
def make_data():    
    ...
    libdata ={
           ...
           't' : pdata.t,
           'x' : tf.transpose(pdata.numerical_sol.y),
           #'y' : pdata.u_test[:,1],
...
def pinn(libdata):
    ...
    model = load_model('stiff_li_model.keras',custom_objects={'custom_loss': custom_loss})
    model.summary()


    #------------not required to fit again
...
def prepare_data(libdata):
    ...
    #array = tf.stack([const_one,x],axis=0)
    ...
    #print(matrix_T)
    y_coeff = tf.cast(tf.ones([1,row])/10,dtype=tf.float64)
...
def lass_fit(libdata):
    ...
    lasso1 = Lasso(alpha=α1, fit_intercept=False, tol=1e-4, max_iter=1000)
    lasso1.fit(iv,dxdt)
    #w1 = np.array(list(lasso1.coef_) + [lasso1.intercept_])
    #w1 = np.concatenate([lasso1.coef_, lasso1.intercept_])
    w1 = lasso1.coef_
...
#----------------end tuning of learning rate--------
    ...
    ut_phi = tf.matmul(
    tf.squeeze(ddt, axis=2),
    tf.squeeze(array, axis=0)
    )
    #print('u_t phi is', ut_phi)
    array = tf.reshape(array, (100, 1))
    phi_phiT = tf.matmul(array, array, transpose_a=True)   # (1,1)
...
#_________to update yij_______
    ...
    #L = min 1/2 ||lambda_T*phi - ddt||^2 + lam ||z|| + sum y_ij(lambda_T-z_T) + rho/2 ||lambda_T-z_T||^2
        ...
        y_predict1 = XX @ lambda_T 
        f1 = np.sum( (dxdt - y_predict1)**2 )
        g1 = np.abs(lambda_T).sum()
        h1 = np.sum(y_coeff*(lambda_T-z_T[0,:]))
        i1 = np.sum((lambda_T-z_T[0,:])**2)
    ...
    y_pred = XX @ tf.transpose(lambda_T)
...
def various_opt(libdata):
    ...
    lambda_sparse = tf.transpose(np.c_[mask_dx])
...
def get_admm_sol(libdata):
    ...
    def func_check(t, x):
        ...
        y1 = lambda_T[0,0]*x[0] #lambda_T[0,0] + lambda_T[0,2]*x[1] + lambda_T[0,3]*(x[0]**2) + lambda_T[0,4]*(x[1]**2) + lambda_T[0,5]*(x[0]*x[1]) 
        #y2 = lambda_T[1,0]*x[0] #lambda_T[1,0]+ lambda_T[1,2]*x[1] + lambda_T[1,3]*(x[0]**2) + lambda_T[1,4]*(x[1]**2) + lambda_T[1,5]*(x[0]*x[1])
               
        return np.array([y1])
    
    sol_lambda = scint.solve_ivp(fun=func_check, t_span=(0,0.01), y0=[1000], method="RK45", t_eval=np.linspace(0,0.01,100))
...
def get_LU_sol(libdata):
    ...
    def func_check(t, x):
        ...
        y1 =  lams[0,0]*x[0] #lams[0,0] + lams[0,2]*x[1] + lams[0,3]*(x[0]**2) + lams[0,4]*(x[1]**2) + lams[0,5]*(x[0]*x[1]) 
        #y2 =  lams[1,1]*x[0] #lams[1,0] + lams[1,2]*x[1] + lams[1,3]*(x[0]**2) + lams[1,4]*(x[1]**2) + lams[1,5]*(x[0]*x[1])
    ...
    sol_LU = scint.solve_ivp(fun=func_check, t_span=(0,0.01), y0=[1000], method="RK45", t_eval=np.linspace(0,0.01,100))
...
def get_lasso_sol(libdata):
    ...
    def func_check(t, x):
        ...
        #---------------sparse----------
        y1 =  w[0,0]*x[0] #w[5,0]+ w[1,0]*x[1] + w[2,0]*(x[0]**2) + w[3,0]*(x[1]**2) + w[4,0]*(x[0]*x[1]) 
       # y2 =  w[0,1]*x[0] #w[1,1]+ w[1,1]*x[1] + w[2,1]*(x[0]**2) + w[3,1]*(x[1]**2) + w[4,1]*(x[0]*x[1])
    ...
    sol_sparse = scint.solve_ivp(fun=func_check, t_span=(0,0.01), y0=[1000], method="RK45", t_eval=np.linspace(0,0.01,100))
...
def visualize(libdata):
    ...
    ax7.plot(output[:,0],'x', label = 'Data(LSODA)')    #(100,2)
    ax7.legend(loc='upper right')
    ax7.plot(sol_sparse.y[0,:], label='Data(lasso)')
    ax7.set_xlabel('Time, t')#, fontdict=font)
    ax7.set_ylabel('Solution, y')#, fontdict=font)
    ax7.legend(loc='upper right')
    ...
    ax8.set_ylabel('Solution, y')#, fontdict=font)
    ax8.legend(loc='upper right')
    fig.savefig('x trajectory.png', dpi=300)
