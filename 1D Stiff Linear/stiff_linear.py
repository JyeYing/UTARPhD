...
def setting_model(data):
    ...
    model.add(Dense(16, activation=tf.keras.activations.gelu, kernel_initializer="glorot_normal", dtype=tf.float64))
    model.add(Dense(8, activation=tf.keras.activations.gelu, kernel_initializer="glorot_uniform", dtype=tf.float64))
    model.add(Dense(1, activation=None, kernel_initializer="glorot_normal",dtype=tf.float64))
    ...
    # cluster near 0 (your stiff region)
    t_f_adapt = 0.01 * (s**3)

    t_f_adapt = tf.convert_to_tensor(t_f_adapt.reshape(-1,1), dtype=tf.float64)
    
    #g = tf.Variable(t_f, dtype = 'float64', trainable = False)
    g = tf.convert_to_tensor(t_f_adapt)
    tau = 10000 * g
    ...
    with tf.GradientTape(persistent=False) as tape:
        ...
        u_eval = model(tau)
        #print(u_eval)
    grads = tape.jacobian(u_eval, tau)  
        #print(grads.values)
    ...
    #f1 = dx_dt - ddt[:,0]   # does not work because LSODA also suffer from stiff
    ...
    model.save('stiff_li_model.keras')
...
def visualize(data):
    ...
    ax5.set_title("1D Stiff Linear")
    # Display the plot
    plt.show()
    fig.savefig('1D_Stiff.png', dpi=300)
    ...
    ax6.set_title("1D Stiff Linear_PINNs")
    # Display the plot
    plt.show()
    fig.savefig('1D_pinn_Stiff.png', dpi=300)
    
    #fig,ax6 = plt.subplots(figsize=(10, 8))
    fig,ax7 = plt.subplots(1,1)
    # Plot the trajectory
    ax7.plot(t,numerical_sol.y[0,:],'x' , label='Data(LSODA)') #2
    ax7.legend(loc='upper right')
    ax7.plot(t,u_test[:,0], label='Data(PINNs)') #
    ax7.legend(loc='upper right')
    # Set labels and title
    ax7.set_xlabel("Time, t")
    ax7.set_ylabel("Solution, y(t)")
    #ax7.set_title("1D Stiff Linear")
    # Display the plot
    plt.show()
    fig.savefig('1D_both_Stiff.png', dpi=300)
    

def save_output(data, fname="linear_stiff.pkl"):
...
def main():
    ...
    visualize(data)
    save_output(data, fname="stiff_linear.pkl")
