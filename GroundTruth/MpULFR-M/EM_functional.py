...
def visualize(libdata):
    ...
    ax1.plot(output[:,0], output[:,1],'x', label = 'Data(DOSM)')    #(100,2)
    ax1.legend(loc='upper left')
    ax1.plot(sol_func.y[0,:],sol_func.y[1,:],'x', label='Data(MpULFR-M)')
    ax1.xlim = (0,10)
    ax1.ylim = (-5,0)
    ax1.set_xlabel('Female, x (in thousands)')#, fontdict=font)
    ax1.set_ylabel('Male, y (in thousands)')#, fontdict=font)
    ax1.legend(loc='upper left')
    fig.savefig('Mp trajectory comparison.png', dpi=300)

    fig,ax17 = plt.subplots(1,1)
    #fig.suptitle("MpULFR-M_Forecast Total Population", fontsize=12)
    ax17.plot(forecast_fm[:,0], forecast_fm[:,1],'x', label = 'Data(DOSM)')    #(100,2)
    ax17.legend(loc='upper left')
    ax17.plot(sol_future.y[0,:],sol_future.y[1,:],'x', label='Data(MpULFR-M)')
    ax17.set_xlabel('Forecasted Female, x (in thousands)')#, fontdict=font)
    ax17.set_ylabel('Forecasted Male, y (in thousands)')#, fontdict=font)
    ax17.legend(loc='upper left')
    fig.savefig('Mp forecast xy.png', dpi=300)
    
    fig,ax18 = plt.subplots(1,1)
    #fig.suptitle("MpULFR-M_Forecast Female Population", fontsize=12)
    ax18.plot(fore_t, forecast_fm[:,0], 'x', label = 'Data(DOSM)')
    ax18.plot(fore_t, sol_future.y[0], 'x', label = 'Data(MpULFR-M)')
    #ax18.set_title('forecast_female, x')
    ax18.legend(loc='upper left')
    ax18.set_xlabel('Year, t')#, fontdict=font)
    ax18.set_ylabel('Forecasted Female, x (in thousands)')#, fontdict=font)
    ax18.legend(loc='upper left')
    fig.savefig('Mp forecast x.png', dpi=300)
    
    fig,ax19 = plt.subplots(1,1)
    #fig.suptitle("MpULFR-M_Forecast Male Population", fontsize=12)
    ax19.plot(fore_t, forecast_fm[:,1], 'x', label = 'Data(DOSM)')
    ax19.plot(fore_t, sol_future.y[1], 'x', label = 'Data(MpULFR-M)')
    #ax19.set_title('forecast_male, y')
    ax19.legend(loc='upper left')
    ax19.set_xlabel('Year, t')#, fontdict=font)
    ax19.set_ylabel('Forecasted Male, y (in thousands)')#, fontdict=font)
    ax19.legend(loc='upper left')
    fig.savefig('Mp forecast y.png', dpi=300)
