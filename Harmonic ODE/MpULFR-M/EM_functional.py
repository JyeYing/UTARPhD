...
def get_functional_sol(libdata):
    ...
    def func_check(t, x):
        ...
        #---------------sparse----------
        y1 = w[0,0]*x[0] + w[1,0]*x[1] + w[2,0]*(x[0]**2) + w[3,0]*(x[1]**2) + w[4,0]*(x[0]*x[1]) 
        y2 = w[1,1]*x[0] + w[2,1]*x[1] + w[2,1]*(x[0]**2) + w[3,1]*(x[1]**2) + w[4,1]*(x[0]*x[1])
...
def visualize(libdata):
    ...
    ax1.plot(t,sol_func.y[0,:], label='Data(MpULFR-M)')
    ax1.xlim = (0,10)
    ax1.ylim = (-5,0)
