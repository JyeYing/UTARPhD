# -*- coding: utf-8 -*-
"""
Created on Tue Jan 20 09:02:58 2026
...
def setting_model(data):
    ...
    ddt = RHS(t_f,numerical_neighbour.y)
    print('ddt is',ddt)
    ...
    # f = dx_dt + dy_dt + u_eval #because this is the physics equation
    ...
    loss_bc = tf.reduce_mean(tf.square(y_pred_bc - x_bc))
    
    def custom_loss(y_true, y_pred):
        
        to_take = sampling + N_f - 5
        loss_data = K.mean(K.square(y_true[:to_take] - y_pred[:to_take]))
        ...
        #loss_f = tf.reduce_mean(tf.square(f1))
    ...
    #x_test = sol.y.T[0]
    #y_test = sol.y.T[1]
    #model.fit(t_to_array, y_to_fit, batch_size,epochs)#, validation_data = (x_test, y_test))
    
    history = model.fit(t_to_array, y_to_fit, batch_size,epochs)#, validation_data = (x_test, y_test))
    #plt.plot(history.history['loss'], label='loss')
    #plt.semilogy(history.history['loss'], label='log_loss')
    #plt.legend()
    #plt.show()
    ...
    fig.savefig('simple_logloss.png', dpi=300)
    ...
    #ic_x = tf.reduce_mean(tf.square(sol.y.T - u_test))
...
def visualize(data):
    ...
    # ax1.plot(numerical_neighbour.y[0,:],label='RK45')
    # ax1.legend(loc='upper right')
    # ax1.plot(u_pred[:,0],'x',label='PINNs')
    # ax1.set_xlabel('count')#, fontdict=font)
    # ax1.set_ylabel('x')#, fontdict=font)
    # ax1.legend(loc='upper right')
    # plt.show()
    # fig.savefig('stiff_linear.png', dpi=300)

    # fig,ax2 = plt.subplots(1,1)
    # fig.suptitle("RK45 vs PINNs", fontsize=12)
    # ax2.plot(numerical_sol.y[0,:], label='RK45')
    # ax2.legend(loc='upper right')
    # ax2.plot(u_test[:,0],'x',label='PINNs')
    # ax2.set_xlabel('count')#, fontdict=font)
    # ax2.set_ylabel('x')#, fontdict=font)
    # ax2.legend(loc='upper right')
    # plt.show()
    # fig.savefig('stiff_linear_test.png', dpi=300)
    
    
    fig,ax5 = plt.subplots(1,1)
    # Plot the trajectory
    ax5.plot(t,numerical_sol.y[0,:]) #
    # Set labels and title
    ax5.set_xlabel("Time, t")
    ax5.set_ylabel("Solution, y(t)")
    ax5.set_title("Van der Pol")
    # Display the plot
    plt.show()
    fig.savefig('VdP_num.png', dpi=300)
    
    #fig,ax6 = plt.subplots(figsize=(10, 8))
    fig,ax6 = plt.subplots(1,1)
    # Plot the trajectory
    ax6.plot(sort_t,numerical_neighbour.y[0,:]) #
    # Set labels and title
    ax6.set_xlabel("Time, t")
    ax6.set_ylabel("Solution, y(t)")
    ax6.set_title("Van der Pol_PINNs")
    # Display the plot
    plt.show()
    fig.savefig('VdP_pinns.png', dpi=300)
    
    #fig,ax6 = plt.subplots(figsize=(10, 8))
    fig,ax7 = plt.subplots(1,1)
    # Plot the trajectory
    ax7.plot(t,numerical_sol.y[0,:] , label='Data(Radau)') #2
    ax7.legend(loc='lower right')
    ax7.plot(t,u_test[:,0], label='Data(PINNs)') #
    ax7.legend(loc='lower right')
    # Set labels and title
    ax7.set_xlabel("Time, t")
    ax7.set_ylabel("Solution, x(t)")
    ax7.set_title("Van der Pol")
    # Display the plot
    plt.show()
    fig.savefig('VdP_both.png', dpi=300)
    
    fig,ax8 = plt.subplots(1,1)
    # Plot the trajectory
    ax8.plot(t,numerical_sol.y[1,:],'x', label='Data(Radau)') #2
    ax8.legend(loc='upper right')
    ax8.plot(t,u_test[:,1], label='Data(PINNs)') #
    ax8.legend(loc='upper right')
    # Set labels and title
    ax8.set_xlabel("Time, t")
    ax8.set_ylabel("Velocity, x'(t)")
    #ax8.set_title("Van der Pol")
    # Display the plot
    plt.show()
    fig.savefig('VdP_velocity.png', dpi=300)
    
    #phase plot
    fig,ax9 = plt.subplots(1,1)
    # Plot the trajectory
    ax9.plot(numerical_sol.y[0,:],numerical_sol.y[1,:],'x', label='Data(Radau)') #2
    ax9.legend(loc='upper left')
    ax9.plot(u_test[:,0],u_test[:,1], label='Data(PINNs)') #
    ax9.legend(loc='upper left')
    # Set labels and title
    ax9.set_xlabel("Displacement, x(t)")
    ax9.set_ylabel("Velocity, x'(t)")
    #ax8.set_title("Van der Pol")
    # Display the plot
    plt.show()
    fig.savefig('VdP_phase.png', dpi=300)
    
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    # Displacement
    axes[0].plot(t,numerical_sol.y[0,:] ,'x', label='Data(Radau)') #2
    axes[0].legend(loc='lower right')
    axes[0].plot(t,u_test[:,0], label='Data(PINNs)') #
    axes[0].legend(loc='lower right')
    ...
    # Phase space
    axes[1].plot(t,numerical_sol.y[1,:],'x', label='Data(Radau)') 
    axes[1].legend(loc='upper left')
    axes[1].plot(t,u_test[:,1], label='Data(PINNs)')
    axes[1].legend(loc='upper left')
...
def save_output(data, fname="vdp.pkl"):
...
def main():
    ...
    visualize(data)
    save_output(data, fname="vdp.pkl")
