# -*- coding: utf-8 -*-
"""
Created on Tue Feb 24 21:34:22 2026
...
t_bc = tf.constant([[10]],dtype=tf.float64)
mu = 2
...
def make_data():    
    ...
    libdata ={
           ...
           't' : pdata.t,
           'x_train' : pdata.u_test[:,0],
           'y_train' : pdata.u_test[:,1],
           
           'out_true' : pdata.numerical_sol.y,
           'x' : pdata.numerical_sol.y[0,:],
           'y' : pdata.numerical_sol.y[1,:],
...
def pinn(libdata):
    ...
    def custom_loss(y_true, y_pred):
        ...
        with tf.GradientTape(persistent=True) as tape2:
            ...
            u_t = tape1.gradient(u_eval, g)     # x'
        u_tt = tape2.gradient(u_t, g)      # x'' 
       
        f = u_tt - mu*(1 - u_eval**2)*u_t + u_eval
        loss_f = tf.reduce_mean(tf.square(f))
        
        with tf.GradientTape() as tape:
            tape.watch(t_ini)
            y_pred0 = model(t_ini)       # shape [1,2]
                
        J = tape.jacobian(y_pred0, t_ini)  
        #print(J)
        loss_ic = tf.reduce_mean(tf.square(y_pred0 - y0))
    ...
    model = load_model('vdp_stiff.keras',custom_objects={'custom_loss': custom_loss})
    model.summary()
...
def prepare_data(libdata):
    ...
    array = tf.stack([const_one,x,y,x2,y2],axis=0)
    #array = tf.stack([x],axis=0)
    #print(array)   #(5x100)
    iv = np.c_[x,y,x2,y2]
...
def various_opt(libdata):
    ...
    print('Sparse SI Matrix is', lambda_sparse2)
    ...
    return lambda_sparse, lambda_sparse2
...
def get_admm_sol(libdata):
    ...
    sol_lambda = scint.solve_ivp(fun=func_check, t_span=(t0,t1), y0=y0, method="Radau", t_eval=np.linspace(t0,t1,gap))
...
def get_LU_sol(libdata):
    ...
    sol_LU = scint.solve_ivp(fun=func_check, t_span=(t0,t1), y0=y0, method="Radau", t_eval=np.linspace(t0,t1,gap))
...
def get_SI_sol(libdata):
    ...
    lams2 = libdata['lambda_sparse2']
    
    def func_check(t, x):
        #dxdt = lambda_T[0,4]*x[1] + lambda_T[0,2]*(x[0]*x[0]) + lambda_T[0,3]*(x[0]**4) + lambda_T[0,7]*((x[0]**2)*(x[1]**2)) 
        #dydt = lambda_T[1,1]*x[0] + lambda_T[1,4]*x[1]+lambda_T[1,6]*((x[0]**2)*x[1]) + lambda_T[1,5]*(x[1]**3)     
       
        y1 =  lams2[0,0] + lams2[0,1]*x[0] + lams2[0,2]*x[1] + lams2[0,3]*(x[0]**2) + lams2[0,4]*(x[1]**2) #+ lams2[0,5]*(x[0]*x[1]) 
        y2 =  lams2[1,0] + lams2[1,1]*x[0] + lams2[1,2]*x[1] + lams2[1,3]*(x[0]**2) + lams2[1,4]*(x[1]**2) #+ lams2[1,5]*(x[0]*x[1])
        
        return np.array([y1, y2])
    
    sol_SI = scint.solve_ivp(fun=func_check, t_span=(t0,t1), y0=y0, method="Radau", t_eval=np.linspace(t0,t1,gap))
...
def get_lasso_sol(libdata):
    ...
    def func_check(t, x):
        ...
        #---------------sparse----------
        y1 =  w[4,0] + w[0,0]*x[0] + w[1,0]*x[1] + w[2,0]*(x[0]**2) + w[3,0]*(x[1]**2) #+ w[4,0]*(x[0]*x[1]) 
        y2 =  w[4,1] + w[0,1]*x[0] + w[1,1]*x[1] + w[2,1]*(x[0]**2) + w[3,1]*(x[1]**2) #+ w[4,1]*(x[0]*x[1])
    ...
    sol_sparse = scint.solve_ivp(fun=func_check, t_span=(t0,t1), y0=y0, method="Radau", t_eval=np.linspace(t0,t1,gap))
...
def visualize(libdata):
    ...
    ax4.plot(output[:,0], output[:,1],'x', label='Data(Radau)')  
    ax4.legend(loc='upper left')
    ax4.plot(sparse_u_pinn[:,0],sparse_u_pinn[:,1], label='PINNs')  
    ax4.set_xlabel('Displacement, x(t)')#, fontdict=font)
    ax4.set_ylabel("Velocity, x'(t)")#, fontdict=font)
    ax4.legend(loc='upper left')
    fig.savefig('solutions trajectory.png', dpi=300)
    
    fig,ax5 = plt.subplots(1,1)
    #fig.suptitle("RK45 vs IPINNs", fontsize=12)
    ax5.plot(output[:,0], output[:,1],'x', label = 'Data(Radau)')    #(100,2)
    ax5.legend(loc='upper left')
    ax5.plot(sol_lambda.y[0,:],sol_lambda.y[1,:], label='Data(IPINNs)')
    ax5.set_xlabel('Displacement, x(t)')#, fontdict=font)
    ax5.set_ylabel("Velocity, x'(t)")#, fontdict=font)
    ax5.legend(loc='upper left')
    fig.savefig('ADMM trajectory.png', dpi=300)
    
    fig,ax6 = plt.subplots(1,1)
    #fig.suptitle("RK45 vs SIPINNs", fontsize=12)
    ax6.plot(output[:,0], output[:,1],'x', label = 'Data(Radau)')    #(100,2)
    ax6.legend(loc='upper left')
    ax6.plot(sol_LU.y[0,:],sol_LU.y[1,:],label='Data(MpULFR-M')
    ax6.set_xlabel('Displacement, x(t)')#, fontdict=font)
    ax6.set_ylabel("Velocity, x'(t)")#, fontdict=font)
    ax6.legend(loc='upper left')
    fig.savefig('Mp trajectory.png', dpi=300)
    
    
    #differences between u_train vs lambda_T (PINNs-Sparse) leads to loss_sparse
    fig,ax7 = plt.subplots(1,1)
    ax7.plot(output[:,0], output[:,1],'x', label = 'Data(Radau)')    #(100,2)
    ax7.legend(loc='upper left')
    ax7.plot(sol_sparse.y[0,:],sol_sparse.y[1,:],'x', label='Data(lasso)')
    ax7.set_xlabel('Displacement, x(t)')#, fontdict=font)
    ax7.set_ylabel("Velocity, x'(t)")#, fontdict=font)
    ax7.legend(loc='upper left')
    fig.savefig('lasso trajectory.png', dpi=300)
     
    # fig,ax8 = plt.subplots(1,1)
    # ax8.plot(t, output[:,0],'x', label = 'exact_x')
    # ax8.plot(t, sol_lambda.y[0,:], label = 'x')
    # #ax8.plot(t[0:-1], sol_lambda.y[0,:], 'x', label = 'x')
    # #ax8.set_title('sparse_x')
    # ax8.legend(loc='upper right')
    # ax8.set_xlabel('Time, t')#, fontdict=font)
    # ax8.set_ylabel('Displacement, x(t)')#, fontdict=font)
    # ax8.legend(loc='upper right')
     
    # fig,ax9 = plt.subplots(1,1)
    # ax9.plot(t, output[:,1], 'x', label = 'exact_y')
    # ax9.plot(t, sol_lambda.y[1,:],label = 'y')
    # #ax9.plot(t[0:-1], sol_lambda.y[1,:], 'x', label = 'y')
    # ax9.set_title('sparse_y')
    # ax9.legend(loc='upper right')
    # ax9.set_xlabel('Time, t')#, fontdict=font)
    # ax9.set_ylabel("Velocity, x'(t)")#, fontdict=font)
    # ax9.legend(loc='upper left')
    
    fig,ax10 = plt.subplots(1,1)
    #fig.suptitle("RK45 vs SIPINNs", fontsize=12)
    ax10.plot(output[:,0], output[:,1],'x', label = 'Data(Radau)')    #(100,2)
    ax10.legend(loc='upper left')
    ax10.plot(sol_SI.y[0,:],sol_SI.y[1,:],label='Data(SIPINNs')
    ax10.set_xlabel('Displacement, x(t)')#, fontdict=font)
    ax10.set_ylabel("Velocity, x'(t)")#, fontdict=font)
    ax10.legend(loc='upper left')
    fig.savefig('SI trajectory.png', dpi=300)
    
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    # Displacement
    axes[0].plot(t,output[:,0] ,'x', label='Data(Radau)') #2
    axes[0].legend(loc='lower right')
    axes[0].plot(t,sol_lambda.y[0,:],label='Data(PINNs)') #
    axes[0].legend(loc='lower right')
    axes[0].set_title("VdP: Displacement plot")
    axes[0].set_xlabel("Time,t")
    axes[0].set_ylabel("Displacement, x(t)")

    # Phase space
    axes[1].plot(t,output[:,1],'x', label='Data(Radau)') 
    axes[1].legend(loc='upper left')
    axes[1].plot(t,sol_lambda.y[1,:], label='Data(PINNs)')
    axes[1].legend(loc='upper left')
    axes[1].set_title("VdP: Velocity plot")
    axes[1].set_xlabel("Displacement, x(t)")
    axes[1].set_ylabel("Velocity,x'(t)")
...
def save_output(libdata, fname="vdp_sp.pkl"):
...
def main():
    ...
    save_output(libdata, fname="vdp_sp.pkl")
