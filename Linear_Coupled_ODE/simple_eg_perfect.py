...
#-------------
    ...
    loss = loss_fall + ic_all
    print('loss from PINNs is:', loss)  #data loss
    losses.append(loss)
        
    data['losses'] = losses
    data['u_pred'] = u_pred
    data['u_test'] = u_test
    data['ddt_pinn'] = ddt_pinn
...
def visualize(data):
    ...
    ax1.plot(numerical_neighbour.y[0,:], numerical_neighbour.y[1,:],'x',label='Data(RK45)')
    ax1.legend(loc='upper left')
    ax1.plot(u_pred[:,0],u_pred[:,1],label='Data(PINNs)')
