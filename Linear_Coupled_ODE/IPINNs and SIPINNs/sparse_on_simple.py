...
def make_data():    
    with open ('simple_eg.pkl','rb') as f:
        data = pickle.load(f)
...
def visualize(var_data):
    ...
    ax1.plot(sol_t[0,:], sol_t[1,:],'x', label='Data(RK45)')
    ax1.legend(loc='upper left')
    ax1.plot(u_pinn[:,0],u_pinn[:,1],label='Data(PINNs)')  
    ax1.set_xlabel('x', fontsize=12)#, fontdict=font)
    ax1.set_ylabel('y', fontsize=12)#, fontdict=font)
    ax1.legend(loc='upper left')
    ...
    ax2.plot(sol_t[0,:], sol_t[1,:], label = 'Data(RK45)')    #(100,2)
    ax2.legend(loc='upper left')
    ax2.plot(sol_lambda.y[0,:],sol_lambda.y[1,:],'x', label='Data(IPINNs)') 
    ax2.set_xlabel('x', fontsize=12)#, fontdict=font)
    ax2.set_ylabel('y', fontsize=12)#, fontdict=font)
    ax2.legend(loc='upper left')
    ...
    ax4.semilogy(losses_sparse, label = 'log_losses of Sparse')
...
def main():
    ...
    save_output(var_data, fname="trial_sparse_simple.pkl")
