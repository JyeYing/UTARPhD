...
def make_data():    
    with open ('sign_func.pkl','rb') as f:
        data = pickle.load(f)
...
def compute_data(var_data):
    ...
    array = tf.stack([const_one,x,x2],axis=0)
    #print(array)
...
def train_PINN(var_data):
   ...
   # model.compile(loss='mse', optimizer='adam', metrics=['accuracy'])
    ...
    model = load_model('sign.keras',custom_objects={'custom_loss': custom_loss})
    model.summary()
...
def admm_sol(var_data):
    ...
    def func_check(t, x):
        dxdt = lambda_T[0,0] + lambda_T[0,1]*x[0] + lambda_T[0,2]*(x[0]**2)      
        #dydt = lambda_T[1,0] + lambda_T[1,1]*x[0] + lambda_T[1,2]*(x[0]**2)
...
def visualize(var_data):
    ...
    #fig.suptitle("Data Driven - RK45 vs PINNs", fontsize=12)
    ax1.plot(sol_t[0,:], 'x',label='Data(RK45)')
    ax1.legend(loc='upper left')
    ax1.plot(u_pinn[:,0], label='Data(PINNs)')    
    ax1.set_xlabel('Time, t')#, fontdict=font)
    ax1.set_ylabel('Solution, x(t)')#, fontdict=font)
    ax1.legend(loc='upper left')
    fig.savefig('diff_sign.png', dpi=300)

    #differences between u_train vs lambda_T (PINNs-Sparse) leads to loss_sparse
    fig,ax2 = plt.subplots(1,1)
    #fig.suptitle("Data Driven - RK45 vs IPINNs", fontsize=12)
    ax2.plot(sol_t[0,:], 'x', label = 'Data(RK45)')    #(100,2)
    ax2.legend(loc='upper left')
    ax2.plot(sol_lambda.y[0,:], label='Data(IPINNs)') 
    ax2.set_xlabel('Time, t')#, fontdict=font)
    ax2.set_ylabel('Solution,x(t) ')#, fontdict=font)
    ax2.legend(loc='upper left')
    fig.savefig('diff_ut_sign.png', dpi=300)
    ...
    fig.savefig('logloss_sign_sparse.png', dpi=300)
    ...
    fig.savefig('sign_sparse_spLoss.png', dpi=300)

    #plot trajectory of coupled first order ODE (especially on other time t)
    

def save_output(var_data, fname="sign_sparse.pkl"):
...
def main():
    ...
    save_output(var_data, fname="sign_sparse.pkl")
