# -*- coding: utf-8 -*-
"""
Created on Thu Apr 16 20:12:24 2026
...
t0, t1, gap= 0, 5, 100  #step size 0.01
sampling = 100
x0=[1]
...
def get_numerical_sol(data,
    ...
    sol = scint.solve_ivp(fun, t_span, x0, method, t_eval)
...
def prepare_input_data(data, N_f):
    ...
    y_numerical_idx = np.vstack((x0, y_numerical_idx))    
...
def neighbour_data(data):
    ...
    sol2 = scint.solve_ivp(fun=RHS, t_span=(t0,t1),y0=x0, method = 'RK45',t_eval=t_eval)
...
def visualize(data):
    ...
    ax5.set_ylabel("Solution, u")
    ax5.set_title("sin x")
    # Display the plot
    plt.show()
    fig.savefig('sign_func.png', dpi=300)
    ...
    # ax6.set_ylabel("Solution, y")
    # ax6.set_title("1D Stiff Linear_PINNs")
    # # Display the plot
    # plt.show()
    # fig.savefig('sinx_neigh.png', dpi=300)
    
    #fig,ax6 = plt.subplots(figsize=(10, 8))
    fig,ax7 = plt.subplots(1,1)
    # Plot the trajectory
    ax7.plot(t,numerical_sol.y[0,:],'x' , label='Data(RK45)') #2
    ...
    # Display the plot
    plt.show()
    fig.savefig('1 over x.png', dpi=300)
    

def save_output(data, fname="sign_func.pkl"):
...
def main():
    ...
    visualize(data)
    save_output(data, fname="sign_func.pkl")
